...
def download_podcasts(api_key, api_secret, ai_episode_count, non_ai_episode_count):
    ...
    for podcast in podcasts:
        ...
        episode_count = ai_episode_count if result['ai_only'] else non_ai_episode_count
    ...
    print("New Episodes before update:")
    print(new_episodes)

    # Update 'downloaded' column for new episodes if the .mp3 file exists
    for idx, row in new_episodes.iterrows():
        # Construct the file path
        file_path = os.path.join(base_dir, "mp3", row['feed_title'], f"{row['id']}.mp3")

        # Check if the file exists
        file_exists = os.path.isfile(file_path)

        # If the file exists, update the 'downloaded' column
        if file_exists:
            new_episodes.at[idx, 'downloaded'] = 1

    print("New Episodes after update:")
    print(new_episodes)

    num_to_download = new_episodes[new_episodes['downloaded'] == 0].shape[0] + df_downloaded[df_downloaded['downloaded'] == 0].shape[0]
    num_existing_downloads = new_episodes[new_episodes['downloaded'] == 1].shape[0] + df_downloaded[df_downloaded['downloaded'] == 1].shape[0]
...
'''
To fix: Issue when mp3 is downloaded, but df_downloaded df does not exist/is empty because csv not saved from before
'''